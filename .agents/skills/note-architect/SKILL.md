...
## Synthesis Process
1. **Research (if needed)**: If only a topic name is given, use web search and official documentation (e.g., Spring.io, Nodejs.org) to gather accurate, current details. Verify version-gated claims (`Java X+` tags, JEP status, deprecated APIs) rather than relying on memory.
...
3. **Structure & Density**: Map the content to the mandatory Content Progression defined in the standards, and allocate depth by subtopic importance per the **Importance-Weighted Coverage** rule and @.agents/rules/weighting.md (expand core, keep useful tight, omit trivia). Consolidate related subtopics and code examples to prevent boilerplate and file bloating.
